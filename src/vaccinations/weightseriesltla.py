...
class WeightSeriesLTLA:
    ...
    def __init__(self):
        ...
        # the source files
        self.source_path = os.path.join(os.getcwd(), 'warehouse', 'vaccinations', 'weights', 'segments', 'ltla')
        self.source_files = glob.glob(os.path.join(self.source_path, '*', '*.csv'), recursive=True)

        # storage
        path = os.path.join(os.getcwd(), 'warehouse', 'vaccinations', 'weights', 'series', 'ltla', 'baseline')
